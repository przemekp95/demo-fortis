...
<template>
    ...
    <div class="font-body relative min-h-screen overflow-hidden bg-slate-950 text-slate-100">
        ...
        <div class="relative z-10">
            <header
                class="sticky top-0 z-30 border-b border-white/10 bg-slate-950/70 backdrop-blur-xl"
            >
                <div
                ...
                >
                    <div class="flex items-center gap-3">
                        <div
                            class="logo-glow flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-300/40 bg-slate-900/80"
                        >
                            <svg
                                viewBox="0 0 28 28"
                                class="h-6 w-6 text-cyan-200"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <path
                                    d="M6 7.5L14 3L22 7.5V16.5L14 21L6 16.5V7.5Z"
                                    stroke="currentColor"
                                    stroke-width="1.8"
                                />
                                <path
                                    d="M14 3V11M22 7.5L14 11M6 7.5L14 11"
                                    stroke="currentColor"
                                    stroke-width="1.8"
                                />
                                <path
                        ...
                        <div>
                            <p
                                class="font-display text-xs uppercase tracking-[0.28em] text-cyan-300"
                            >
                                FORTIS
                            </p>
                            <p class="text-base font-semibold text-white sm:text-lg">
                                Loteria Paragonowa 2026
                            </p>
                        </div>
                    </div>
                ...
                <transition name="mobile-panel">
                    <div
                        v-if="mobileMenuOpen"
                        class="border-t border-white/10 bg-slate-950/95 px-4 py-4 sm:px-6 lg:hidden"
                    >
                        <div class="mx-auto max-w-7xl space-y-4">
                            <nav class="grid gap-2 text-sm">
                                <a href="#jak-dziala" class="mobile-link" @click="closeMobileMenu"
                                    >Jak to działa</a
                                >
                                <a href="#nagrody" class="mobile-link" @click="closeMobileMenu"
                                    >Nagrody</a
                                >
                                <a href="#wyniki" class="mobile-link" @click="closeMobileMenu"
                                    >Wyniki</a
                                >
                                <Link
            ...
            <main
                class="mx-auto flex max-w-7xl flex-col gap-10 px-4 pb-10 pt-14 sm:px-6 sm:pt-16 lg:px-8 lg:pt-20"
            >
                <section class="grid gap-6 lg:grid-cols-[1.35fr_1fr]">
                    <div
                        class="reveal rounded-3xl border border-white/15 bg-slate-900/70 p-7 shadow-2xl shadow-cyan-950/40 sm:p-9"
                    >
                        <p
                            class="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.15em] text-cyan-200"
                        >
                        ...
                        <h1
                            class="font-display mt-4 text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl"
                        >
                            Jeden paragon. Jedno zgłoszenie. Realna szansa na wygraną.
                        </h1>

                        <p
                            class="mt-5 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-lg"
                        >
                            Weź udział w loterii Fortis, zgłoś paragon online i śledź status swoich
                            zgłoszeń w czasie rzeczywistym. Losowania odbywają się codziennie oraz w
                            finale kampanii.
                        </p>
                        ...
                        <div class="mt-8 grid gap-3 sm:grid-cols-3">
                            <div class="rounded-xl border border-white/10 bg-slate-950/70 p-4">
                                <p class="text-xs uppercase tracking-[0.1em] text-slate-400">
                                    Tryb
                                </p>
                    ...
                    <aside
                        class="reveal delay-1 rounded-3xl border border-white/15 bg-slate-900/70 p-7 sm:p-8"
                    >
                        <figure
                            class="mb-6 overflow-hidden rounded-2xl border border-white/15 bg-slate-950/70"
                        >
                            <img
                        ...
                        <h2 class="font-display text-2xl font-semibold text-white">
                            {{ campaign?.name ?? 'Brak aktywnej kampanii' }}
                        </h2>
                        ...
                        <div class="mt-6 rounded-2xl border border-white/10 bg-slate-950/70 p-4">
                            <div
                                class="mb-2 flex items-center justify-between text-xs uppercase tracking-[0.1em] text-slate-400"
                            >
                        ...
                        <dl class="mt-6 space-y-3 text-sm">
                            <div
                                class="flex items-center justify-between gap-4 border-b border-white/10 pb-2"
                            >
                            ...
                            >
                                ...
                                <dd class="text-right font-medium text-white">
                                    {{ formatDate(campaign?.final_draw_at) }}
                                </dd>
                            </div>
                            <div
                                class="flex items-center justify-between gap-4 border-b border-white/10 pb-2"
                            >
                                ...
                                <dd class="text-right font-medium text-white">
                                    {{ campaign?.rule?.max_entries_per_day ?? 'Brak danych' }}
                                </dd>
                            </div>
                            <div
                                class="flex items-center justify-between gap-4 border-b border-white/10 pb-2"
                            >
                                ...
                                <dd class="text-right font-medium text-white">
                                    {{ formatMoney(campaign?.rule?.min_purchase_amount) }}
                                </dd>
                            </div>
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <dd class="text-right font-medium text-white">
                                    {{
                                        campaign?.rule?.max_receipt_age_days
                                            ? `${campaign.rule.max_receipt_age_days} dni`
                                            : 'Brak danych'
                                    }}
                                </dd>
                            </div>
                ...
                <section class="reveal delay-2 grid gap-5 lg:grid-cols-2">
                    <figure
                        class="overflow-hidden rounded-3xl border border-white/15 bg-slate-900/70"
                    >
                        <img
                            src="/images/landing/hero-prizes.svg"
                    ...
                    <div class="mt-6 grid gap-4 md:grid-cols-3">
                        <article class="rounded-2xl border border-white/10 bg-slate-950/70 p-5">
                            ...
                            <p class="mt-2 text-sm leading-relaxed text-slate-300">
                                Rejestracja wymaga podstawowych danych i akceptacji zgód. Po
                                aktywacji maila możesz wysyłać paragony.
                            </p>
                        </article>
                        <article class="rounded-2xl border border-white/10 bg-slate-950/70 p-5">
                            ...
                            <p class="mt-2 text-sm leading-relaxed text-slate-300">
                                Podaj numer paragonu, kwotę i datę zakupu. Silnik antyfraudowy
                                ocenia ryzyko i nadaje status zgłoszeniu.
                            </p>
                        </article>
                        <article class="rounded-2xl border border-white/10 bg-slate-950/70 p-5">
                            <p class="font-display text-4xl font-bold text-cyan-300">03</p>
                            <h3 class="mt-3 text-lg font-semibold text-white">
                                Śledź wyniki losowań
                            </h3>
                            <p class="mt-2 text-sm leading-relaxed text-slate-300">
                ...
                >
                    ...
                    <div class="mt-6 grid gap-4 md:grid-cols-2">
                        ...
                        >
                            <p class="text-xs uppercase tracking-[0.1em] text-slate-300">
                                {{ drawTypeLabel(prize.draw_type) }}
                            </p>
                            <h3 class="mt-2 text-xl font-semibold text-white">{{ prize.name }}</h3>
                            <div
                                class="mt-4 flex items-center justify-between text-sm text-slate-200"
                            >
                ...
                <section id="wyniki" class="grid gap-6 xl:grid-cols-2">
                    <article
                        class="reveal delay-3 rounded-3xl border border-white/15 bg-slate-900/70 p-7 sm:p-8"
                    >
                                ...
                                <div class="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-slate-300">
                                            {{ winner.winner.email }}
                                            <span v-if="winner.winner.city">
                                                · {{ winner.winner.city }}</span
                                            >
                                        </p>
                                    </div>
                                    <p class="text-xs text-slate-400">
                                        {{ formatDateTime(winner.published_at) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <p class="font-semibold text-white">
                                            {{ drawTypeLabel(draw.type) }} #{{ draw.id }}
                                        </p>
                                        <p class="mt-1 text-sm text-slate-300">
                                            Liczba zwycięzców: {{ draw.winner_count }}
                                        </p>
                                    </div>
                                    <p class="text-xs text-slate-400">
                                        {{ formatDateTime(draw.finished_at || draw.started_at) }}
                                    </p>
                                </div>
                            </div>
                        </div>
...
.bg-grid {
    background-image: linear-gradient(to right, rgba(148, 163, 184, 0.11) 1px, transparent 1px),
